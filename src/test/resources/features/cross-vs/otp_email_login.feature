@not_implemented @vs-identity @cross-vs @vs-customer-communications @segment_1 @j1.2 @b2c @otp @authentication @login @critical @api @messaging @integration @event-driven @e2e
Feature: Envio de OTP via Email - Fluxo Cross-VS Completo (LOGIN)
  Como um usuário autenticado
  Eu quero receber OTP via email durante o login
  ...
  @otp_request @cross-vs-email @login
  Scenario: Envio de OTP via Email - Fluxo Cross-VS Completo (LOGIN)
    Given que existe um usuário com email do usuário criado e senha do usuário criado
    ...
    And o Delivery Tracker Service deve registrar o envio com status "SENT"
